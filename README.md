...
> 🤖 Hecho por The Carlos, pensado para ayudar a las personas sin internet,todos los que quieren un bot completo y diferente usa black clover IA totalmente gratis.
...
<details>
  <summary><b>✞ Comandos para mantener 24/7 el Bot black-clover-MD </b></summary>

> *Ejecutar estos comandos dentro de la carpeta black-clover-MD*
```bash
...
pm2 logs 
``` 
> *Si deseas detener la ejecución de Termux use:*
```bash 
pm2 stop index
``` 
> *Si deseas iniciar de nuevo la ejecución de Termux use:*
```bash 
pm2 start index
```

---- 
### En caso de apagarse o detenerse
> _Si despues que ya instalastes el bot y termux te salta en blanco, se fue tu internet o reiniciaste tu celular, solo realiza estos pasos:_
```bash
...
> *Detén el bot, haz click en el símbolo (ctrl) [default=z] usar la letra "z" + "ENTER" hasta que salga algo verdes similar a: `black-clover-MD $`*
> **Escribe los siguientes comandos uno x uno :**
```bash 
cd && cd black-clover-MD && rm -rf sessions/Principal && npm run qr
```

----
### Obtener nuevo código de teléfono 
```bash 
cd && cd black-clover-MD && rm -rf sessions/Principal && npm run code
```
</details>

<details>
<summary><b>✞ Actualizar black-clover-MD</b></summary>

> **Utiliza esta opción únicamente si deseas actualizar a la última versión de black-clover-MD.pero ten en cuenta que al usarla se eliminarán todos los archivos de la versión actual y se reemplazarán con los de la nueva versión. Solo se conservará la base de datos, por lo que será necesario volver a vincular el Bot o usa el comando copia para descargar las database.json ahí encontraras todos los usuarios registrados.**  

**Comandos para actualizar black-clover-MD  de forma automática**
...
```
**✰ Volverte owner del Bot Black-clover-MD**

*Si después de instalar el bot e iniciar la sesión (deseas poner tu número es la lista de owner pon este comando:*

```bash
cd && cd black-clover-MD && nano config.js
```
#### Para que no pierda su progreso en black-clover-MD, estos comandos realizarán un respaldo de su `database.json` y se agregará a la versión más reciente.

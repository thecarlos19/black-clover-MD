...
function isSpam(user, limit = 8, interval = 60000) {
  let now = Date.now();
  let data = global.cmdLimiter[user] || { count: 0, time: now };

  if (now - data.time > interval) {
    data.count = 1;
    data.time = now;
  } else {
    data.count++;
  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      if (typeof plugin.before === 'function') {
        let before = await plugin.before.call(this, m, {
          match,
          conn: this,
          participants,
          groupMetadata,
      ...
      if ((usedPrefix = (match?.[0] || [''])[0])) {
        ...
        let mini = !!(plugin?.botAdmin || plugin?.admin || plugin?.group || noPrefix || m.text?.slice(0, 1));
    ...
    if (m) {
      ...
      utente.exp += m.exp || 0;
      utente.monedas = Math.max(0, utente.monedas - (m.monedas || 0));
    }
...
try {
let dead = !userr?.ws || !userr.ws.socket || userr.ws.socket.readyState === ws.CLOSED
if (dead) {
if (userr.ws?.socket) {
try { userr.ws.socket.close() } catch {}
}
if (typeof userr.logout === 'function') {
try { await userr.logout() } catch {}
}
if (typeof userr.end === 'function') {
try { userr.end() } catch {}
}
if (typeof userr.connect === 'function') {
await userr.connect()
console.log(chalk.cyan("Subbot reiniciado"))
}
}
} catch {}
}}})
...
try {
let dead = !conn?.ws || !conn.ws.socket || conn.ws.socket.readyState === ws.CLOSED
if (dead) {
console.log(chalk.yellow("Subbot muerto, reiniciando"))
if (conn.ws?.socket) {
try { conn.ws.socket.close() } catch {}
}

...
import { makeWASocket as makeSimpleSocket } from './simple.js';
  ...
  const server = http.createServer(async (req, res) => {
      ...
      res.end(`
        ...
        <head>
          ...
          <style>
            ...
            .toggle-group button { flex: 1; padding: 10px; background: #2a2a2a; border: 2px solid #333; border-radius: 6px; color: #fff; cursor: pointer; font-size: 14px; }
            .toggle-group button.active { border-color: #8b5cf6; background: #2a1a4a; }
          </style>
        </head>
        <body>
          <div class="container">
            <h1>⚔️ Sub Bot</h1>
            <p>Conecta tu número como Sub-Bot</p>
            ...
            <button id="connectBtn" onclick="startPairing()">Iniciar conexión</button>
            
            <div class="loading" id="loading">
              <div class="spinner"></div>
              <p style="margin-top: 10px; color: #a0a0a0;">Conectando...</p>
            </div>
            
            <div class="status" id="status"></div>
            
            <div class="qr-container" id="qrContainer">
              <h3 style="color: #8b5cf6; margin: 0 0 10px;">📲 Escanea el QR</h3>
              <img id="qrImage" src="" alt="QR Code" />
              <p style="color: #a0a0a0; font-size: 12px; margin-top: 8px;">El código expira en 60 segundos</p>
            </div>
          ...
          <script>
            ...
            async function startPairing() {
              ...
              if (!phone) {
                showStatus('error', 'Por favor ingresa un número de teléfono');
                return;
              }
              ...
              try {
                ...
                if (data.ok) {
                  ...
                  if (data.connected) {
                    showStatus('success', 'Bot conectado como ' + data.number);
                  ...
                  if (data.type === 'qr' && data.qr) {
                    ...
                    showStatus('info', 'Escanea el QR con WhatsApp');
                    startStatusCheck(sessionId);
                  } else if (data.type === 'code' && data.code) {
                    document.getElementById('codeDisplay').textContent = data.code;
                    document.getElementById('codeDisplay').style.display = 'block';
                    showStatus('info', 'Ingresa este código en WhatsApp');
                    startStatusCheck(sessionId);
                  }
                } else {
                  showStatus('error', 'Error ' + data.message);
                  btn.disabled = false;
                  btn.textContent = 'Iniciar conexión';
                }
              } catch (error) {
              ...
              statusInterval = setInterval(async () => {
                ...
                try {
                  ...
                  if (data.connected) {
                    ...
                    document.getElementById('connectBtn').disabled = false;
                    document.getElementById('connectBtn').textContent = 'Iniciar conexión';
                  }
                } catch (error) {
    ...
    try {
      if (method === 'POST' && pathname === '/pairing') {
        ...
        if (!phone) {
          return sendJson(res, 400, { ok: false, message: 'Número de teléfono requerido' });
        }
        
        const pho = normalizePhoneForPairing(phone);
        if (!pho) {
          return sendJson(res, 400, { ok: false, message: 'Número inválido' });
        }

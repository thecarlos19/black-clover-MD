...
if (!global.rutaJadiBot) {
  global.rutaJadiBot = path.join(__dirname, '../núcleo•clover/blackJadiBot');
}
...
export default async (sock) => {
  ...
  async function createSocket(phone) {
    ...
    const dir = path.join(global.rutaJadiBot, pho);
  ...
  async function requestPairingCode(rawPhone, type = 'qr') {
    ...
    if (type === 'code') {
      try {
        ...
        return { connected: false, error: 'Error al generar código: ' + error.message };
      }
    } else {
      return new Promise((resolve) => {
        let timeoutId;

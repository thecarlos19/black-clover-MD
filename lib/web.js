...
const getHtmlPath = () => {
  ...
  path.join(process.cwd(), 'lib', 'index.html'),
  path.join(__dirname, './lib/index.html')
];
...
function getMusicList() {
  const musicPath = path.join(process.cwd(), 'lib', 'music.json');
  try {
    if (fs.existsSync(musicPath)) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map(song => ({
          ...
          artist: song.artista || song.artist || 'Desconocido',
          album: song.album || 'Single',
          year: song.año || song.year || 0,
          genre: song.genero || song.genre || 'Otro',
          duration: song.duracion || song.duration || '0:00',
...
async function loadExistingSessions() {
  try {
    if (fs.existsSync(pathblackJadiBot)) {
      const dirs = fs.readdirSync(pathblackJadiBot);
      for (const dir of dirs) {
        const fullPath = path.join(pathblackJadiBot, dir);
        if (fs.statSync(fullPath).isDirectory()) {
          ...
          if (fs.existsSync(credsPath)) {
            ...
            const tempSocket = {
              ...
              isInit: true,
              uptime: Date.now(),
              user: { id: dir + '@s.whatsapp.net' }
            };
            global.subBotSessions.set(jid, tempSocket);
            if (!global.conns.find(c => c.userId === dir)) {
              global.conns.push(tempSocket);
            }
          }
        }
      }
...
export default async (sock) => {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // Archivos estáticos (CSS, JS, imágenes, fuentes, favicon, etc.)
if (method === 'GET') {
  const staticFile = path.join(process.cwd(), 'lib', pathname);

...
async function loadHandler() {
  try {
    const handlerPath = path.join(process.cwd(), 'núcleo•clover', 'handler.js');
    if (fs.existsSync(handlerPath)) {
      const handler = await import(`file://${handlerPath}?update=${Date.now()}`);
      cachedHandler = handler.default || handler;
  ...
  async function createSocket(phone, mcode = false) {
    ...
    let creloadHandler = async function (restatConn) {
      try {
        ...
        if (Handler && Object.keys(Handler).length) cachedHandler = Handler;
      } catch {}
        ...
        s.ev.on("messages.upsert", async (msg) => {
          try {
            ...
            for (const message of messages) {
              if (cachedHandler && typeof cachedHandler.handler === 'function') {
                await cachedHandler.handler(s, message);
              }
            }
          } catch (error) {

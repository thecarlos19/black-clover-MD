...
const CURRENCY = 'monedas'; // etiqueta para mostrar
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  // normaliza owners: puede ser ["123"], [["123","Nombre"]], etc.
  ...
  const costo = (unit.includes('dia') || unit === 'd')
    ? Math.ceil(minutes / 1440) * 240000      // 1 día = 240,000
    : Math.ceil(minutes * (50000 / 60));      // 1 hora = 50,000
  ...
  user.monedas -= costo;
  user.premium = true;
  // si ya tenía premium, se acumula el tiempo
...
// Verifica expiración y owner en cada mensaje
